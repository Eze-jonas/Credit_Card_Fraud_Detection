# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 20:05:49 2024
...
k = 4  # Number of folds
param_grid = {'max_depth': [3, 5, 10, None], 'min_samples_split': [2, 5, 10]}  # Example pruning parameters

# Define StratifiedKFold for preserving class distribution across folds
cv = StratifiedKFold(n_splits=k)

# Decision Tree with GridSearchCV for optimal pruning
tree = DecisionTreeClassifier(random_state=42)
grid_search = GridSearchCV(tree, param_grid, cv=cv, scoring='f1', n_jobs=-1)
...
train_acc, train_prec, train_rec, train_f1, y_train_pred = evaluate_model(best_tree, X_train, y_train, "Training Set")

# Cross-Validation predictions and metrics using cross_val_predict
y_cv_pred = cross_val_predict(best_tree, X_train, y_train, cv=cv)
...
val_acc, val_prec, val_rec, val_f1, y_val_pred = evaluate_model(best_tree, X_val, y_val, "Validation Set")

# Evaluate on the Test Set
print("Test Set Evaluation:")
test_acc, test_prec, test_rec, test_f1, y_test_pred = evaluate_model(best_tree, X_test, y_test, "Test Set")
...
# Training ROC Curve
train_auc = plot_roc_curve(best_tree, X_train, y_train, "Training Set")
# Cross-Validation ROC Curve using cross_val_predict with probabilities
y_cv_prob = cross_val_predict(best_tree, X_train, y_train, cv=cv, method='predict_proba')[:, 1]
...
val_auc = plot_roc_curve(best_tree, X_val, y_val, "Validation Set")
# Test ROC Curve
test_auc = plot_roc_curve(best_tree, X_test, y_test, "Test Set")
...
joblib.dump(best_tree, r'C:\Users\USER\Desktop\credit_card_fraud_detection\action_project_flask_app\best_decision_tree_model.joblib')  # Save the trained model to the specified path
print("Decision Tree model saved to 'C:\\Users\\USER\\Desktop\\credit_card_fraud_detection\action_project_flask_app\\best_decision_tree_model.joblib'.")
...
metrics_df.to_csv(r'C:\Users\USER\Desktop\credit_card_fraud_detection\decision_tree_model_evaluation_metrics.csv', index=False)
print("Evaluation metrics saved to 'C:\\Users\\USER\\Desktop\\credit_card_fraud_detection\\decisin_tree_model_evaluation_metrics.csv'.")
